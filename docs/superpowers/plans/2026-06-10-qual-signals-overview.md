# Qual Signals — Overview Page Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Fix the broken `QualOutput` TypeScript type and add a "Señales Estratégicas" section to the Overview page showing sentiment gauge, fortalezas/riesgos, and factores de crecimiento.

**Architecture:** Two sequential tasks — first fix the TS type so the compiler is happy, then add the new section to Overview.tsx. All new rendering logic lives inline in Overview.tsx; no new component files needed (the section is small enough to stay in one file).

**Tech Stack:** React 19, TypeScript, Tailwind v4, shadcn/ui (Card), lucide-react
...
dashboard/src/
├── types/
│   └── analysis.ts        ← MODIFY: fix QualOutput, add QualSignals
└── pages/
    └── Overview.tsx        ← MODIFY: add Señales Estratégicas section
```

---

## Task 1: Fix QualOutput TypeScript type

**Files:**
- Modify: `dashboard/src/types/analysis.ts`

- [ ] **Step 1: Replace the `QualOutput` block in `analysis.ts`**

Find these lines (currently around line 82–85):
...
Expected: zero errors. (The Overview page currently accesses `result.qual.signals` as `Signal[]` — that reference will break. That's intentional: Task 2 fixes it. If you see only Overview.tsx errors, that's fine — proceed. If you see errors in other files, fix them before committing.)

- [ ] **Step 3: Commit**

```bash
cd "/Users/josuetapiahernandez/Documents/6_Semestre/Optimizacion_No_Lineal/FLI Holdings"
git add dashboard/src/types/analysis.ts
git commit -m "fix(types): correct QualOutput to match Python model"
```

---

## Task 2: Add Señales Estratégicas to Overview page

**Files:**
- Modify: `dashboard/src/pages/Overview.tsx`

- [ ] **Step 1: Replace `dashboard/src/pages/Overview.tsx` with the full implementation**
...
git commit -m "feat(dashboard): Señales Estratégicas section on Overview page"
```
